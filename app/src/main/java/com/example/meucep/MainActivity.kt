...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Aqui está a parte nova para limpar o texto
                ...
                retrofit.setEndereco(cep).enqueue(/* callback = */ object : Callback<Endereco>{
                    override fun onResponse(call: Call<Endereco>, response: Response<Endereco>) {
                        if (response.code() == 200){
                            ...
                            Toast.makeText(applicationContext, "CEP Errado, Digite novamente!",Toast.LENGTH_SHORT).show()
                            binding.editBairro.setText("")
                            binding.editCidade.setText("")
                            binding.editEstado.setText("")
                            binding.editLogradouro.setText("")
                        }
                        if (binding.editCEP.text.toString() == "00000000") {
                            Toast.makeText(applicationContext, "CEP Errado, Digite novamente!",Toast.LENGTH_SHORT).show()
                            binding.editCEP.setText("")
                            binding.editBairro.setText("")
                            binding.editCidade.setText("")
